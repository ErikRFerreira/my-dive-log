...
export function buildDiveInsightPrompt(input: BuildDiveInsightPromptInput): string {
  const { dive, profile, signals, metrics, baselines } = input;
  const hasBaseline =
    baselines.availability.hasLocationBaseline ||
    baselines.availability.hasGlobalBaseline ||
    baselines.availability.hasRecentBaseline;
  const toneInstruction = inferToneInstruction(profile.certificationLevel);
  const banList = [
    'Nitrox + calm conditions likely contributed to a safe dive.',
    'Wetsuit provided thermal protection.',
    'Good visibility improves the experience.',
  ];
...
Recommendations must be an array when present (can be empty).
...
9) ${toneInstruction}
10) Ban these vague fillers entirely: ${banList.join(' | ')}.
11) dive_insight.text MUST reference at least one provided comparison from metrics.comparisons/topComparison OR use "${NO_MEANINGFUL_INSIGHT_TEXT}".

Historical comparison rule (mandatory):
- hasBaseline: ${hasBaseline ? 'true' : 'false'}
- Priority for baseline_comparison: location (if available) -> global (if available) -> recent (if available) -> "${NO_BASELINE_COMPARISON}".
- If hasBaseline=true, baseline_comparison MUST come from metrics.topComparison or another listed comparison.
...
baselines = ${JSON.stringify(baselines, null, 2)}
  `.trim();
}
